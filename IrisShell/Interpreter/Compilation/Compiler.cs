...
namespace IrisShell.Interpreter
{
    internal sealed class Compiler
    {
        ...
        public CompiledExpression Compile(Expression expression)
        {
            ...
            {
                int i = 0;
                CompiledCommand[] sequence = new CompiledCommand[expression.Count];
                foreach (CommandNode commandNode in expression)
                {
                    sequence[i++] = CompileCommand(commandNode);
                }

                return new CompiledCommandSequence(sequence);
            }
        ...
        private IArgument CompileArgument(Node node)
        {
            return node switch
            {
                CommandNode commandNode => new CompiledInlineCommand(CompileCommand(commandNode)),
